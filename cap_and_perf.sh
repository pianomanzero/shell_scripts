...
usage()
{
    ...
    echo "  -g : Collect a gcore of process(s) specified, comma separated (ie lwio,lsass,netlogon,srvsvc)"
	...
	echo ""
	echo "  Collect 1 hour of performance data along with hangdumps and lwio, lsass, srvsvc, nfsd, mountd, and netlogon gcores:"
	echo "    `basename $0` -i 60 -e 5 -r 12 -d -w -g lwio,lsass,srvsvc,netlogon,nfsd,mountd"
	echo ""
    echo "  When Statistic Collection is done, the output file will be"
    echo "  located in /ifs/data/Isilon_Support/"
    echo ""
	echo ""
    exit
}
...
##############################################
##
# Stop pervious running versions of this script
##
kill_script()
{
...
echo "before the script was stopped is located in /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF"
...
while getopts i:e:r:dg:w opt; do
        case "${opt}"
        in
		...
		g) STATS_GCORE=${OPTARG}
			...
			for i in "${ADDR[@]}"; do
			 if [ $i != "lwio" ] && [ $i != "lsass" ] && [ $i != "srvsvc" ] && [ $i != "netlogon" ] && [ $i != "lwreg" ] && [ $i != "lwsmd" ]; then
			  echo ""
			  echo "ERROR:"
			  echo "The valid options for -g gcore are lwio, lsass, srvsvc or netlogon; they can be combined with a comma. (ie lwio,lsass)"
			  echo ""
			  usage
			 else
...
if [[ -z $STATS_ITERATIONS ]] || [[ -z $STATS_INTERVAL ]] || [[ -z $STATS_REPEAT ]]; then
usage
fi
##
##############################################
...
isi_for_array sysctl isi.stats.client.nfs.max_clients=1024
isi_for_array sysctl isi.stats.client.nfs4.max_clients=1024
...
if [ $ISIHANG_DUMP ]; then
 ...
 echo "Please see KB 43 for more information on how to gather additional Hangdumps if needed."
 echo ""
...
echo "Beginning Pre-Stat Gcore collection, this could take a few of minutes."
			...
			for i in "${ADDR[@]}"; do
			 if [ $i == "lwio" ]; then
			  ...
			  isi_for_array 'pgrep lwio|xargs gcore -s -c /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.lwio.$(date +%m%d%Y_%H%M%S).core'
			elif [ $i == "lsass" ]; then
			  if [ $STATS_DEBUG ]; then echo "Creating $i Cores";fi
			  isi_for_array 'pgrep lsass|xargs gcore -s -c /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.lsass.$(date +%m%d%Y_%H%M%S).core'
			elif [ $i == "netlogon" ]; then
			  if [ $STATS_DEBUG ]; then echo "Creating $i Cores";fi
			  isi_for_array 'pgrep netlogon|xargs gcore -s -c /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.netlogon.$(date +%m%d%Y_%H%M%S).core'
			elif [ $i == "srvsvc" ]; then
			  if [ $STATS_DEBUG ]; then echo "Creating $i Cores";fi
			  isi_for_array 'pgrep srvsvc|xargs gcore -s -c /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.srvsvc.$(date +%m%d%Y_%H%M%S).core'
			elif [ $i == "lwreg" ]; then
			  if [ $STATS_DEBUG ]; then echo "Creating $i Cores";fi
			  isi_for_array 'pgrep lwreg|xargs gcore -s -c /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.lwreg.$(date +%m%d%Y_%H%M%S).core'
			elif [ $i == "lwsmd" ]; then
			  if [ $STATS_DEBUG ]; then echo "Creating $i Cores";fi
			  isi_for_array 'pgrep lwsmd|xargs gcore -s -c /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.lwmsd.$(date +%m%d%Y_%H%M%S).core'  
			elif [ $i == "nfsd" ]; then
                          if [ $STATS_DEBUG ]; then echo "Creating $i Cores";fi
                          isi_for_array 'pgrep nfsd|xargs gcore -s -c /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.nfsd.$(date +%m%d%Y_%H%M%S).core' 
			elif [ $i == "mountd" ]; then
                          if [ $STATS_DEBUG ]; then echo "Creating $i Cores";fi
                          isi_for_array 'pgrep mountd|xargs gcore -s -c /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.mountd.$(date +%m%d%Y_%H%M%S).core'
			else
			  echo "$STATS_GCORE"
		          exit 1
			 fi
...
# sysctrls set in pre-processing along with
...
if [ $ISIHANG_DUMP ]; then
 ...
 isi_for_array -s 'ls /var/crash/isi_hangdump_$(date +%Y_%m_%d)* | cut -d / -f 4 | while read foo; do bar=$(cp "/var/crash/$foo" /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$foo);done'
...
# /ifs/data/Isilon_Support directory
##
file_cleanup()
{
...
isi_stats()
{
 ...
 ####
  ...
  { echo $(date +%m%d%Y_%H%M%S);isi statistics protocol --nodes=all --protocols=nfs3,nfs4 --total --interval $STATS_INTERVAL --repeat $STATS_REPEAT --degraded; } >> $LOCATION/All_Nodes.$(date +%m%d%Y_%H%M%S)_nfs_latency.out1 &
 ...
 # Network Stats
 ####
 if [ $STATS_DEBUG ]; then echo "isi statistics history node.net.ext.bytes.in.rate,node.net.ext.bytes.out.rate (Network In/Out Throughput per ndoe)";fi
...
#######
  ...
  if [ -f $LOCATION/stats_done.tmp1 -a -f $LOCATION/array_done.tmp2 ]
  then
   post_processing
  else
...
isi_array()
{
 ...
 isi_for_array '{ echo $(date +%m%d%Y_%H%M%S);isi_cache_stats -v } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_cache.out2 &'

 if [ $STATS_DEBUG ]; then echo "isi_for_array sysctl vfs.nfsrv (NFS sysctls) $(($(date +%s) - start_time2)) seconds";fi
 isi_for_array '{ echo $(date +%m%d%Y_%H%M%S);sysctl vfs.nfsrv } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_SYSCTL_NFS.out2 &'

 if [ $STATS_DEBUG ]; then echo "top output for lwio (lwio cpu utilization per node) $(($(date +%s) - start_time2)) seconds";fi
 isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);top -H -P -u } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_cpu.out2 &'
 isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);top -S 30 } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_cpu_top.out2 &'

 if [ $STATS_DEBUG ]; then echo "sysctl kern.proc.all_stacks $(($(date +%s) - start_time2)) seconds";fi
 isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);sysctl kern.proc.all_stacks } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_kern_proc_all_stacks.out2 &'

 if [ $STATS_DEBUG ]; then echo "Session List $(($(date +%s) - start_time2)) seconds";fi
 if [ $ONEFS_VERSION == "v7" ]; then
   isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);echo "Session List:"; isi smb session list | grep "Total"; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_session_list.out2 &'
 else
   isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);echo "Session List:"; isi smb session list | grep "Session" | wc -l; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_session_list.out2 &'
 fi

 if [ $STATS_DEBUG ]; then echo "SMB Open File List $(($(date +%s) - start_time2)) seconds";fi
 if [ $ONEFS_VERSION == "v7" ]; then
   isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);echo "File List: ";isi smb openfiles list | grep "Total"; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_file_list.out2 &'
 else
   isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);echo "File List: ";isi smb file list | grep "File" | wc -l; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_file_list.out2 &'
 fi

 if [ $STATS_DEBUG ]; then echo "NFS NLM Lists $(($(date +%s) - start_time2)) seconds";fi
 if [ $ONEFS_VERSION == "v7" ]; then
   isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);echo "Lock List: ";isi nfs nlm locks list | grep "Total"; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_nfs_nlm_list.out2 &'
   isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);echo "Lock Waiter List: ";isi nfs nlm locks waiters | grep "Total"; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_nfs_nlm_list.out2 &'
   isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);echo "Sessions List: ";isi nfs nlm sessions list | grep "Total"; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_nfs_nlm_list.out2 &'
 else
   isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);echo "Clients List: ";isi nfs clients list | grep -v "No clients" | wc -l; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_nfs_nlm_list.out2 &'
   isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);echo "Lock List: ";isi nfs locks list | grep -v "No locks"|wc -l;} >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_nfs_nlm_list.out2 &'
   isi_for_array  '{ echo $(date +%m%d%Y_%H%M%S);echo "Lock Wiater List: ";isi nfs locks waiters| grep -v "No waiters"|wc -l; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_nfs_nlm_list.out2 &'
 fi


 if [ $STATS_DEBUG ]; then echo "checking if debug logging is on $(($(date +%s) - start_time2)) seconds";fi
 isi_for_array '{ echo $(date +%m%d%Y_%H%M%S);echo lwio Logging:;isi smb log-level;echo lsass Logging:;isi auth log-level; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_log_level.out2 &'

 if [ $STATS_DEBUG ]; then echo "Gathering netstat commands $(($(date +%s) - start_time2)) seconds";fi
 isi_for_array '{ echo $(date +%m%d%Y_%H%M%S);netstat -na; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_netstat_na.out2 &'
 isi_for_array '{ echo $(date +%m%d%Y_%H%M%S);netstat -in; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_netstat_in.out2 &'
 isi_for_array '{ echo $(date +%m%d%Y_%H%M%S);netstat -s; } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_netstat_s.out2 &'

 if [ $STATS_DEBUG ]; then echo "checking memory utilization $(($(date +%s) - start_time2)) seconds";fi
 isi_for_array '{ echo $(date +%m%d%Y_%H%M%S);echo lwio Memory:;ps uaxfwww | grep "lwio";echo lsass Memory:;ps uaxfwww | grep "lsass";echo SrvSvc Memory:;ps uaxfwww | grep "s[r]vsvc" } >> /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.$(date +%m%d%Y_%H%M%S)_process_memory.out2 &'
...
#######
 ...
 if (( $TOTAL_ARRAY_DELAY <= $ARRAY_DELAY ))
 then
 ...
 ####
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_SYSCTL_NFS.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.NFS.sysctls.txt'
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_cache.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.GENERAL.cache.txt'
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_cpu.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.GENERAL.cpu.txt'
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_cpu_top.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.GENERAL.cpu_top.txt'
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_kern_proc_all_stacks.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.GENERAL.kern_proc_all_stacks.txt'
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_session_list.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.SMB.session_list.txt'
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_file_list.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.SMB.file_list.txt'
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_nfs_nlm_list.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.NFS.nlm_list.txt'
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_log_level.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.SMB.log_level.txt'
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_netstat_na.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.NETWORK.netstat_na.txt'
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_netstat_in.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.NETWORK.netstat_in.txt'
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_netstat_s.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.NETWORK.netstat_s.txt'
 isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_process_memory.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.GENERAL.service_memory.txt'
...
#######
   ...
   # Add a cat statement for the stat added
   # in Step 1 in the following format:
   # isi_for_array -s 'cat /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.*_lwio_cpu.out2 > /ifs/data/Isilon_Support/$(date +%m%d%Y)_ISIPERF/`hostname`.lwio_cpu.txt'
#######
